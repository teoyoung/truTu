...
export default class Telegram {
    ...
    constructor(token: string) {
        ...
        bot.onText(/\/start/, (msg) => {
            bot.sendMessage(msg.chat.id, message_start, {parse_mode: 'Markdown'});
        });
        ...
        bot.onText(/\/add (.+)/, (msg, [command, data]) => {
            ...
            if (isNaN(uid)) {
                bot.sendMessage(id, message_incorrect_id);
                return;
            }

            if (!key || key !== addKey) {
                bot.sendMessage(id, message_incorrect_key);
                return;
            }

            if (!name_book) {
                bot.sendMessage(id, message_incorrect_name_book);
                return;
            }
            ...
            database.addBook(dataBook, 
                () => { bot.sendMessage(id, AddBookMessage('Complete', {uid, name_book }), {parse_mode: 'Markdown'})}, 
                () => {
                    bot.sendMessage(id, AddBookMessage('Warning', {uid, name_book }), {parse_mode: 'Markdown'})
                    bot.sendMessage(warning, `Warning: ${AddBookMessage('Warning', {uid, name_book })}`, {parse_mode: 'Markdown'})
                },
                () => {
        ...
        bot.onText(/\/take (.+)/, (msg, [command, data]) => {
            ...
            database.takeBook(dataBook, (idOwner, nameBook) => {
        ...
        bot.onText(/\/return (.+)/, (msg, [command, data]) => {
            ...
            database.returnBook({tgID_user: id, uid, username},
                (name_book, ownerID) => {
        ...
        bot.onText(/\/all/, (msg) => {
            ...
            database.allBooks(
                (books) => bot.sendMessage(msg.chat.id, books, {parse_mode: 'Markdown'})
            );
        });
        ...
        bot.onText(/\/top/, (msg) => {
            ...
            database.getTop(
                (topDonat, topUsers, bookTop) => {
                    ...
                    bot.sendMessage(msg.chat.id, TopMessage('*Топ владельцев книг*', messageTop), {parse_mode: 'Markdown'});
                    bot.sendMessage(msg.chat.id, TopMessage('*Топ читателей книг*', messageTopUsers), {parse_mode: 'Markdown'});
                    bot.sendMessage(msg.chat.id, TopMessage('*Топ читаемых книг*', messageTopBook), {parse_mode: 'Markdown'});
                },
                (userError, adminError) => {
                    if(userError){
